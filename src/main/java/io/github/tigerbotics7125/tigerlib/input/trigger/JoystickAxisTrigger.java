...
public class JoystickAxisTrigger extends Trigger {
     ...
     */
    public JoystickAxisTrigger(GenericHID joystick, int axis, double threshold, ThresholdType thresholdType,
	    boolean invert) {
	...
	super(() -> {
	    ...
	    // Java 11
	    /*
	     * switch (thresholdType) { case Exact: return val == threshold;
	     * case LessThan: return val < threshold; case GreaterThan: return
	     * val > threshold; case Deadband: return Math.abs(val) > threshold;
    ...
    /**
     * Create a new {@link JoystickAxisTrigger} representing the same joystick
     * axis, but with a different threshold condition.
     *
